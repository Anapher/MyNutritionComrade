...
namespace MyNutritionComrade.Core.Utilities
{
    ...
    {
        public static void PatchServings(Product product, IEnumerable<ProductServing> servings, IEnumerable<ProductServingDto> sourceServings)
        {
            bool CompareItems(ProductServingDto arg1, ProductServing arg2)
            {
            ...
            void RemoveItem(ProductServing obj)
            {
                product.RemoveProductServing(obj.Id);
            }

            void UpdateItem(ProductServing arg1, ProductServingDto arg2)
            {
        ...
        public static void PatchLabels(Product product, IEnumerable<ProductLabel> productLabels, IEnumerable<LocalizedLabel> labels)
        {
            ListPatch.PatchStrings(labels, productLabels, (x, y) => x.LanguageCode == y.LanguageCode,
                (localizedLabel, label) => new[] {(label.Name, localizedLabel.Label)}, label => product.RemoveProductLabel(label.Id),
        ...
        private static IEnumerable<(string, string)> GetKeys(ItemLocalizedLabel arg1, ProductServingLabel arg2)
        {
