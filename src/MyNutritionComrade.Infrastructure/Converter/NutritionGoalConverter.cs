...
namespace MyNutritionComrade.Infrastructure.Converter
{
    ...
    public class NutritionGoalConverter : JsonConverter<NutritionGoalBase?>
    {
        private static readonly IReadOnlyDictionary<Type, string> NutritionGoalTypeMap = new Dictionary<Type, string>
        {
            {typeof(CaloriesFixedNutritionGoal), "caloriesFixed"},
        ...
        public override void WriteJson(JsonWriter writer, NutritionGoalBase? value, JsonSerializer serializer)
        {
            if (value == null)
            {
                writer.WriteNull();
        ...
        public override NutritionGoalBase? ReadJson(JsonReader reader, Type objectType, NutritionGoalBase? existingValue, bool hasExistingValue, JsonSerializer serializer)
        {
            var obj = JObject.Load(reader);
            ...
            return (NutritionGoalBase?) obj.ToObject(type);
        }
    }
}
