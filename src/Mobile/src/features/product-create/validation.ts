...
const schema = z
   .object({
      ...
      nutritionalInfo: z
         ...
         .refine(
            (val) => val.sugars === undefined || val.carbohydrates === undefined || val.sugars <= val.carbohydrates,
            {
   ...
   .refine((val) => val.servings[val.defaultServing] ?? 0 > 0, {
      path: ['defaultServing'],
      message: 'Default serving must be a valid serving',
   });
